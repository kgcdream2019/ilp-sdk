...
const testAddRemove = (createUplink) => async (t) => {
...
test('btc: add then remove', testAddRemove(helpers_1.addBtc()));
test('eth: add then remove', testAddRemove(helpers_1.addEth()));
test('xrp: add then remove', testAddRemove(helpers_1.addXrp()));
/*newly added code for xmrd*/
test('xmrd: add then remove', testAddRemove(helpers_1.addXmrd()));
/*end*/
...
test('eth: cannot add duplicate uplink', async (t) => {
    ...
    await t.throwsAsync(helpers_1.addEth()(t.context));
});
test('xrp: cannot add duplicate uplink', async (t) => {
    await helpers_1.addXrp()(t.context);
    await t.throwsAsync(helpers_1.addXrp()(t.context));
});
test('btc: cannot add duplicate uplink', async (t) => {
    await helpers_1.addBtc()(t.context);
    await t.throwsAsync(helpers_1.addBtc()(t.context));
});
/*newly added code for xmrd*/
test('xmrd: cannot add duplicate uplink', async (t) => {
    await helpers_1.addXmrd()(t.context);
    await t.throwsAsync(helpers_1.addXmrd()(t.context));
});
/*end*/
...
test('add with invalid xrp secret throws', async (t) => {
    await t.throwsAsync(t.context.add({
...
test('add with invalid eth secret throws', async (t) => {
    await t.throwsAsync(t.context.add({
        ...
        privateKey: 'this is not a valid eth secret despite being the correct leength'
    }));
    // TODO Fix that ->
...
test('add with invalid lnd credentials throws', async (t) => {
    await t.throwsAsync(t.context.add({
        ...
        tlsCert: '',
...
//# sourceMappingURL=add-remove.test.js.map