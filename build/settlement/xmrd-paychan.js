...
const ilp_plugin_xmrd_paychan_1 = __importStar(require("ilp-plugin-xmrd-paychan"));
...
const dropsToXmrd = (amount) => crypto_rate_utils_1.exchangeQuantity(crypto_rate_utils_1.baseQuantity(assets_1.xmrdAsset, amount));
const xmrdToDrops = (amount) => crypto_rate_utils_1.baseQuantity(crypto_rate_utils_1.exchangeQuantity(assets_1.xmrdAsset, amount));
const getXmrdServerWebsocketUri = (ledgerEnv) => ledgerEnv === 'mainnet'
    ? 'wss://s1.ripple.com'
...
exports.closeXmrdPaychanEngine = ({ api }) => api.disconnect();
...
exports.configFromXmrdCredential = ({ address, ...cred }) => cred;
exports.getBaseBalance = async (settler, credential) => {
    const response = await settler.api.getAccountInfo(credential.address);
    return crypto_rate_utils_1.exchangeQuantity(assets_1.xmrdAsset, response.xrpBalance);
};
const connectUplink = (credential) => (state) => async (config) => {
    ...
    const plugin = new ilp_plugin_xmrd_paychan_1.default({
        role: 'client',
        server,
        xmrdServer,
        xmrdSecret: secret
    }, {
        log: log_1.default('ilp-plugin-xmrd'),
        store: new store_1.MemoryStore(store)
    });
    const pluginAccount = await plugin._loadAccount('peer');
    const toXmrd = operators_1.map(amount => dropsToXmrd(amount).amount);
    ...
    rxjs_1.fromEvent(pluginAccount.account.outgoing, 'data')
        .pipe(operators_1.startWith(pluginAccount.account.outgoing.state), operators_1.map(ilp_plugin_xmrd_paychan_1.spentFromChannel), toXmrd)
        .subscribe(totalSent$);
    const outgoingCapacity$ = new rxjs_1.BehaviorSubject(new bignumber_js_1.default(0));
    rxjs_1.fromEvent(pluginAccount.account.outgoing, 'data')
        .pipe(operators_1.startWith(pluginAccount.account.outgoing.state), operators_1.map(ilp_plugin_xmrd_paychan_1.remainingInChannel), toXmrd)
    ...
    rxjs_1.fromEvent(pluginAccount.account.incoming, 'data')
        .pipe(operators_1.startWith(pluginAccount.account.incoming.state), operators_1.map(ilp_plugin_xmrd_paychan_1.spentFromChannel), toXmrd)
        .subscribe(totalReceived$);
    const incomingCapacity$ = new rxjs_1.BehaviorSubject(new bignumber_js_1.default(0));
    rxjs_1.fromEvent(pluginAccount.account.incoming, 'data')
        .pipe(operators_1.startWith(pluginAccount.account.incoming.state), operators_1.map(ilp_plugin_xmrd_paychan_1.remainingInChannel), toXmrd)
        .subscribe(incomingCapacity$);
    return {
        settlerType: engine_1.SettlementEngineType.XmrdPaychan,
        asset: assets_1.xmrdAsset,
...
const deposit = (uplink) => (state) => async ({ amount, authorize }) => {
    ...
    await uplink.pluginAccount.fundOutgoingChannel(fundAmountDrops, async (feeXmrd) => {
        ...
        await authorize({
            value: amount,
            fee: crypto_rate_utils_1.exchangeQuantity(assets_1.xmrdAsset, feeXmrd)
        });
    });
    ...
    const isAuthorized = new Promise((resolve, reject) => {
        ...
        claimChannel = uplink.pluginAccount
            ...
            const internalAuthorize = authorize({
                value: uplink.outgoingCapacity$.value.plus(dropsToXmrd(channel.spent).amount),
                fee: crypto_rate_utils_1.exchangeQuantity(assets_1.xmrdAsset, feeXmrd)
            });
...
//# sourceMappingURL=xmrd-paychan.js.map